...
LastEditors: Fantasy
LastEditTime: 2020-11-03 23:00:31
Descripttion: 
...
class Report(object):
    ...
    def login(self):
        ...
        if 'success' == resp.text:
            print("用户 {} 第 {} 次登陆成功".format(self.username, self.login_count))
            return True
        elif 'fail' == resp.text:
            print("用户 {} 密码错误，请检查配置文件".format(self.username))
            return False
        elif self.login_count < 3:
            print("用户 {} 第 {} 次登陆失败，正在重试".format(self.username, self.login_count))
            return self.login()
        else:
            print("用户 {} 第 {} 次登陆失败，超过重试次数，请检查系统...".format(self.username, self.login_count))
            print(resp.text)
            return False
    ...
    def apply_info_extract(self):
        ...
        if re.findall(r'提交', resp.text):
            ...
            print("用户 {} 今日已上报过".format(self.username))
            return False
    ...
    def send_report(self):
        ...
        if report_data:
            ...
            if re.findall(r'true', resp.text):
                print("用户 {} 上报成功".format(self.username))
            else:
                print("用户 {} 上报失败：".format(self.username))
                print(resp.text)
    ...
    def logout(self):
        ...
        if re.findall(r'请输入用户名', resp.text):
            print("用户 {} 注销成功".format(self.username))
        else:
            print("用户 {} 注销失败：".format(self.username))
            print(resp.text)
        self.session.close()
